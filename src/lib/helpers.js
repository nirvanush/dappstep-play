...
export function friendlyToken(token, quantity = true, length = 13) {
  ...
  res += token.tokenId.slice(0, length) + '...' + token.tokenId.slice(-length) + ' token';
  return res;
}
...
export function addForKey(req, key) {
  ...
  if (reqs.length < 100) {
    if (reqs.map((cur) => cur.id).includes(req.id)) return;
  }
...
export function parseQueries(query) {
  ...
  queries.forEach((query) => {
    let cur = query.split('=');
    if (cur[0].length > 0) queryMp[cur[0]] = decodeURI(cur[1]);
  });
  if (!Object.keys(queryMp).includes('artist')) queryMp['artist'] = undefined;
  if (!Object.keys(queryMp).includes('searchValue')) queryMp['searchValue'] = undefined;
  return queryMp;
}
