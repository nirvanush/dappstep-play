...
export async function follow(request) {
    return await post(getUrl(assmUrl) + '/follow', request)
        .then((res) => res.json())
        .then((res) => {
...
export async function p2s(request) {
    return await post(getUrl(assmUrl) + '/compile', request)
        .then((res) => res.json())
        .then((res) => {
            if (res.success === false) throw new Error();
            return res;
    ...
    for (let i = 0; i < favs.length; i++) {
        ...
        if (newBid.id !== favs[i].boxId) {
            if (newBid.address === auctionAddress) {
                ...
                addNotification(
                    `New bid for your favorite auction "${newBid.assets[0].name}"`,
                    getAuctionUrl(newBid.id),
                );
            } else {
                addNotification(
                    `Your favorite auction ${newBid.assets[0].name} has ended`,
                    getAuctionUrl(newBid.id),
                );
        ...
        if (newBid.address === auctionAddress && !favs[i].remNotifDone) {
            ...
            if (rem <= remFavNotif) {
                addNotification(
                    `Your favorite auction "${newBid.assets[0].name}" is near the end`,
                    getAuctionUrl(newBid.id),
                );
    ...
    for (let i = 0; i < bids.length; i++) {
        ...
        if (box.spentTransactionId) {
            ...
            if (tx.outputs[0].address === auctionAddress)
                addNotification(
                    `You've been outbidded for the "${bids[i].name}" auction`,
                    getAuctionUrl(tx.outputs[0].id),
                );
            else
                addNotification(
                    `Congrats! You've won the "${bids[i].name}" auction`,
                    getAuctionUrl(bids[i].id),
                );
        }
    }
}
    ...
    for (let i = 0; i < auctions.length; i++) {
        ...
        if (newBid.id !== auctions[i].id) {
            ...
            if (newBid.address === auctionAddress) {
                ...
                addNotification(
                    `New bid with amount of ${longToCurrency(
                        bidAmount,
                        null,
                        currency,
                    )} ${currency} for your auction "${auctions[i].name}" is placed`,
                    getAuctionUrl(newBid.id),
                );
            } else {
...
export async function pendings() {
    ...
    for (let i = 0; i < bids.length; i++) {
        try {
            ...
            const txs = (await txByAddress(addr)).filter(
                (tx) => tx.inputs.map((inp) => inp.address).includes(addr) && tx.outputs.length > 2,
            );
            ...
            if (txs.length > 0) {
                ...
                if (bid.key === 'bid') {
                    msg = `Your bid for ${longToCurrency(bid.amount, null, bid.box.currency)} ${bid.box.currency
                        } on "${bid.box.tokenName}" is placed`;
                    addForKey(
                        {
                    ...
                    addBid({
                        ...
                        status: 'complete',
                        amount:
                            tx.outputs[0].assets.length === 1
                                ? tx.outputs[0].value
                                : tx.outputs[0].assets[1].amount,
                        txId: tx.id,
                    });
                } else {
                ...
                if (!bid.unc) {
                    ...
                    if (unc.tx) {
                        ...
                        if (tx.outputs.length === 2) {
                            // refund
                            removeForKey('pending', bid.id);
                            addNotification('Your funds are being returned!', getTxUrl(tx.id), 'error');
                        } else {
                            bid.unc = true;
                            let msg = 'Your auction is starting';
                            if (bid.key === 'bid')
                                msg = `Your bid for ${longToCurrency(bid.amount, null, bid.box.currency)} ${bid.box.currency
                                    } on "${bid.box.tokenName}" is being placed`;
            ...
            if (past > 120) removeForKey('pending', bid.id);
        } catch (e) { }
    }
}
...
export async function myArtworks() {
    ...
    for (let i = 0; i < artworks.length; i++) {
        try {
            ...
            if (past > 120) removeForKey('my-artworks', artworks[i].id);
        } catch (e) { }
    }
}
...
export async function assembleFinishedAuctions(boxes) {
    const toWithdraw = boxes.filter(
        (box) =>
            box.remTimeTimestamp <= 0 || (box.curBid >= box.instantAmount && box.instantAmount !== -1),
    );
    ...
    for (let i = 0; i < toWithdraw.length; i++) {
        ...
        if (box.curBid < box.minBid) {
            ...
            const auctionFee = Math.floor(
                (box.curBid * parseInt(dataInput.additionalRegisters.R4.renderedValue)) / 1000,
            );
            ...
            let artistAddr = null;
            if (!isP2pkAddr(artBox.ergoTree) && artistFee > 0)
                artistAddr = await getReturnAddr(artBox.address);
        ...
        if (Object.keys(request).length > 0)
            post(getUrl(assmUrl) + '/follow', request)
                .then((res) => res.json())
                .then((res) => {
