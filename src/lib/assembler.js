...
export async function outBid() {
  ...
  for (let i = 0; i < bids.length; i++) {
    ...
    if (box.spentTransactionId) {
      ...
      else addNotification(`Congrats! You've won the "${bids[i].name}" auction`, getAuctionUrl(bids[i].id));
    }
  }
}
  ...
  for (let i = 0; i < auctions.length; i++) {
    ...
    if (newBid.id !== auctions[i].id) {
      ...
      if (newBid.address === auctionAddress) {
        ...
        addNotification(
          `New bid with amount of ${longToCurrency(bidAmount, null, currency)} ${currency} for your auction "${
            auctions[i].name
          }" is placed`,
  ...
  for (let i = 0; i < bids.length; i++) {
    try {
      ...
      if (txs.length > 0) {
        ...
        if (bid.key === 'bid') {
          msg = `Your bid for ${longToCurrency(bid.amount, null, bid.box.currency)} ${bid.box.currency} on "${
            bid.box.tokenName
          }" is placed`;
          addForKey(
            {
...
export async function assembleFinishedAuctions(boxes) {
  const toWithdraw = boxes.filter(
    (box) => box.remTimeTimestamp <= 0 || (box.curBid >= box.instantAmount && box.instantAmount !== -1),
  );
