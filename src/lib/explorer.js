...
export function getUnconfirmedTxsFor(addr) {
  return getRequest(`/mempool/transactions/byAddress/${addr}`, explorerApiV1).then((res) => res.items);
}
...
export function getAuctionHistory(limit, offset, auctionAddr) {
  return getRequest(`/addresses/${auctionAddr}/transactions?limit=${limit}&offset=${offset}`, explorerApiV1).then(
    (res) => res.items,
  );
}
...
export async function followAuction(id) {
  ...
  while (cur.spentTransactionId) {
    ...
    if (new_cur.address === auctionAddress) cur = new_cur;
    else break;
  }
